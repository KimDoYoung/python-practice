...
ELIANA_MODE = os.getenv('MELIAN_MODE', 'LOCAL')
ELIANA_PORT=int(os.getenv('ELIANA_PORT', 8989))

# ELIANA_MODE 값에 따라 CHART_BASE_URL을 설정합니다.
if ELIANA_MODE == "LOCAL":
    CHART_BASE_URL = f"http://localhost:{ELIANA_PORT}"
    LOG_FILE_NAME = os.path.join("c:\\tmp\\logs\\eliana.log")
else:  # ELIANA_MODE가 "REAL"일 경우, 실제 운영 환경의 URL을 사용합니다.
    CHART_BASE_URL = "https://localhost:{ELIANA_PORT}"
