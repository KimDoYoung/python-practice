...
Lucy는 파이썬언어(version 3.12.2)를 기반으로 개발되었으며 현재 한국펀드서비스(주)의 AssetERP의  메인 개발 언어인 java 기반의 기술과는 다른
...
![ipo-hisotory](./images/ipo-history.png)
  ...
  - 사용자의 한국투자증권 계좌의 계좌정보(예수금등)와 보유 종목의 목록과 종목의 현재가 등을 표시합니다.
  - 만약 매수/매도의 현재가가 아닌 금액으로 주문을 넣었을 경우 오늘의 미체결에 표시합니다.
  
  - 주식 일별주문체결
  ![일별주문체결](./images/kis-chegeol.png)
  - KIS계좌에서 체결된 정보를 일자별로 조회합니다.
  
  ### 4.3.5 LS증권
  
  - LS증권 계좌에 대한 정보를 표시합니다.
  
  - **LS계좌**
  ![LS계좌](./images/ls-account.png)
  - 사용자의 LS계좌에 대한 예수금등의 정보와 보유 종목의 목록과 종목의 현재가 등을 표시합니다.
  
  - **LS증권 거래내역**
  ![LS거래내역](./images/ls-chegeul.png)
  - LS계좌에서 체결된 정보 및 거래내역을 조회합ㅈ니다.

  ### 4.3.6 매수/매도

  - 매수/매도는 모든 메뉴에서 호출될 수 있도록 구현되어 있으며 bootstrap의 canvas를 이용해서 UI를 구현했습니다.
  ![매수매도](./images/buysell.png)
  - 증권사를 선택하고 상품번호(stock code)를 입력 후 수량과 금액을 입력한 후 매수/매도 버튼을 클릭함으로서 해당 증권사에 주문을 넣게 됩니다.
  - 이때 금액이 0일 경우 시장가로 주문이 들어가게 됩니다.
  - 현재 메뉴에 따라서 증권사가 자동선택될 수 있으며 매수/매도 수량은 기본적으로 1로, 시장가 0으로 설정됩니다.
  
  ### 4.3.7 fulldown 오른쪽 메뉴

  - fulldown 메뉴의 오른쪽 메뉴는 문자열이 아니라 아이콘으로 이루어져 있습니다.
  ![right-menu](./images/right-menu.png)
  -순서대로 다음과 같은 기능입니다.
  1. 별표 : 나의 주식(My Stock)
  2. 새로고침 : 각 메뉴마다 현재가가 있는 경우 모든 페이지에서 자동으로 갱신되지 않을 경우에 사용가능
  3. 날짜와 요일 : 거래일와 거래시간에 따라서 각 화면이 달리 작용할 수 있습니다.
  4. 설정 : 시스템 설정 사항와 스케줄러 설정 현황
  5. 사용자 : 사용자 정보

  ### 4.3.8 설정

  ![config](./images/config.png)  
  - Lucy는 상수값에 해당하는 값을 .env<profile>과 몽고db의 config collection에 넣어 두고 사용합니다.
  - 몽고db에서 관리하는 설정값을 조회, 편집가능합니다.
  - 설정값들은  Config collection에 저장됩니다.
  - 몽고db에서 관리하는 설정값
    - ip_expected_cost_express : IPO의 예상체결가 공식 (IPO 과거기록에서 산출, IPO목록에서 예상체결가 산출)
    - 38-scrapping-time : 커뮤니케이션38에서 scrapping한 최종 시각 (Scheduler job site38_work에서 산축, IPO목록에서 사용)

  - 또한 설정메뉴에서는 Scheduler의 Job 리스트를 조회 편집할 수 있습니다
    - scheduler job의 동작시간은 cron형태이며 편집가능합니다.
    - 설정된 cron job들
      - site38_work : [커뮤니케이션38](https://www.38.co.kr/) 사이트 스크랩
      - holiday_godata : [공공데이터](https://www.data.go.kr/)에서 제공하는 휴일정보
      - LS_stk_info채우기 : LS정보의 종목정보, LS증권은 6자리의 stkcode외에 자신들이 지정한 코드를 api에서 사용하도록 강제함으로 이 정보를 채워두었다가 사용하게 됨.
      - juda_fetch : [주달](https://www.judal.co.kr/)사이트에서 scrapping

  ### 4.3.9 사용자정보

  ![user](./images/user.png)
  - 사용자 정보를 조회, 편집할 수 있습니다.
  - 사용자 정보는 몽고db의 Users collection에 저장됩니다.
  - 사용자의 기본정보 : 로그인시 사용
  - 키-값 : 사용자의 open api 신청 후 받은 키 값
    - 공공데이터에서 발급
    - DART에서 발급
    - Telegram 에서 생성
  - KIS, LS의 계좌번호 및 암호
  - KIS, LS의 APP_KEY와 SECRET_KEY
  - KIS, LS의 TOKEN은 발급받은 후 발급시간과 함께 저장되어 24시간 이내에서는 이 값을 사용하게 됩니다.
