...
class StockWsManager:
    ...
    async def disconnect(self, user_id: str, stk_company_abbr: str):
        if user_id in self.stock_connections and stk_company_abbr in self.stock_connections[user_id]:
            ...
            self.client_ws_manager.send_personal_message(f"사용자 {user_id}의 모든 증권사 WebSocket 작업 종료", user_id)
            logger.debug(f"사용자 {user_id}의 모든 증권사 WebSocket 작업 종료")
    ...
    async def stock_ws_task(self, user_id: str, stk_company_abbr: str):
        ...
        if stk_company_abbr == "kis":
            kis_task = KISTask(user_id, self.client_ws_manager)
            await kis_task.run()
    ...
    def status(self):
        ...
        for user_id, connections in self.stock_connections.items():
            user_status = {}
            for stk_company_abbr, task in connections.items():
                user_status[stk_company_abbr] = str(task.get_coro().__name__)
