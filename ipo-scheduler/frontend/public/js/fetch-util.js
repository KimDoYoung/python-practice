 ...
 */
class IPOScheduelerError extends Error {
 ...
 */
async function callIpoSchedulerApi(url, method, data = null) {
    const token = localStorage.getItem('ipo_scheduler_token');
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                throw new IPOScheduelerError(response.status, errorData.detail);
            } else {
                throw new IPOScheduelerError(response.status, 'Unexpected response format');
            }            
        }
        ...
        if (errorStr.includes('SyntaxError: Unexpected end of JSON input')) {
            ...
            throw new IPOScheduelerError(500, errorStr);
        }
    }
}
...
async function getFetch(url) {
    return callIpoSchedulerApi(url, 'GET');
}
...
async function postFetch(url, data) {
    return callIpoSchedulerApi(url, 'POST', data);
}
...
async function putFetch(url, data) {
    return callIpoSchedulerApi(url, 'PUT', data);
}
...
async function deleteFetch(url, data) {
    return callIpoSchedulerApi(url, 'DELETE', data);
}
...
async function fetch_handlebar(hbs_file_path) {
    ...
    if (!response.ok) {
        throw new IPOScheduelerError(500, "Network response was not ok " + response.statusText);
    }
